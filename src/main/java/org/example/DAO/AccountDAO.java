...
public class AccountDAO extends BaseDAO<BankAccount> {
    ...
    public boolean update(BankAccount element) throws SQLException {
        ...
        statement.setDouble(1, element.getTotalAmount()); // On "set" par rapport au paramétres BankAccount ligne 31
        statement.setInt(2, element.getId()); // On "set" par rapport au paramétres BankAccount ligne 31
        int nbRow = statement.executeUpdate();
        return nbRow == 1;
    }
    ...
    public BankAccount get(int id) throws SQLException {
        ...
        if(resultSet.next()) { //On récupéere un seul objet donc on utilise le "if"
