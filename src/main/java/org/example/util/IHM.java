...
public class IHM {
    ...
    public void createAccountAction(){
        ...
        if(customer != null) {
            BankAccount bankAccount = bankService.createAndSaveAccount(customer.getId()); //Besoin que de l'id du customer pour créer le compte
            if (bankAccount != null){
    ...
    public void withDrawlAction(){
        ...
        scanner.nextLine();
        if(bankAccount != null){
            if(bankService.makeOperationWithDraw(montant*-1, bankAccount.getId())){ //Le -1 permet de changer le montant en négatif
    ...
    private Customer createCustomerAction(){
        ...
        customer = bankService.createAndSaveCustomer(firstName,lastName,phone); //La methode "createAnd.." permet également de générer un id
        return customer;

    }

    public void getAccountOperations(){ //Pour récupérer le compte via l'id pour eviter de toujours demander l'ID
        ...
        BankAccount bankAccount = bankService.getAccountAction(id);
        if(bankAccount != null){
            for (Operation op : bankService.getAllOpertionByIdAccount(bankAccount.getId())) { //Je vais parcourir ttes les opération via "op" créé et instancié (peut importe le nom)
